...
class stateTrigger():
    ...
    def __repr__(self):
        return repr((self.eventCount, self.trigger, self.state , self.eventMap, self.eventOrder, self.eventOrderCount,self.eventDist))
...
class InputHandler(DirectObject.DirectObject):
    ...
    def setKey(self,eventnr,SetOrClear):
        ...
        #TODO: have an eye on the side-swapping.. it might fight back and jump at you from behind, tearing your spine out and eat your balls. will happen. promise.
        ...
        if SetOrClear:
            ...
            self.keystatus.discard(-eventnr)
        else:
            self.keystatus.add(-eventnr)
            self.keystatus.discard(eventnr)
            
        for trigger in self.permaTriggers:
            trigger.calcEventDist(eventnr) #update the key-distance-weighting for sorting it corretcy in the pollEvents thing.
        
        self.pollEvents(eventnr)
    ...
    def pollEvents(self,eventnr=0):
        ...
        #eventOrderCount is the length of the combo wich order must be corretly pressed to work out
        ...
        for index,trigger in enumerate (sorted(self.permaTriggers, key=attrgetter('eventCount', 'eventOrderCount','eventDist') ,reverse = True) ):
            #print index,trigger
            if trigger.eventMap.issubset(self.keystatus): #if all buttons match the current button-states
